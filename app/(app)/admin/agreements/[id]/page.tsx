...
export default async function AdminAgreementDetailPage({
  ...
  const paidRecorded =
    payment?.status === "SUCCESS" && typeof payment.amount === "number";
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Recorded when the customer completes checkout (demo gateway
              today).
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-2 text-sm">
            {payment ? (
              <>
                ...
                />
                <Row label="Gateway" value={paymentGatewayLabel(payment)} />
                <Row
                  label="Paid / recorded at"
          ...
          <CardContent className="space-y-2 text-sm">
            {agreement.delivery ? (
              <>
                <Row
                  label="Method"
                  value={adminDeliveryMethodLabel(agreement.delivery.method)}
                />
                <Row
                  label="Address"
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Line totals are qty × unit price (rupees, before GST — GST is on
              the full order total at checkout).
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <Row
              label="Stamp duty (order)"
              value={
                agreement.stampValue != null
                  ? formatINR(agreement.stampValue)
                  : "—"
              }
            />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-600">
                        <span>
                          Qty {o.qty} × {formatINR(o.unitPrice)}
                        </span>
...
function paymentGatewayLabel(
  payment: {
    provider: string;
    mockTxnId: string | null;
    providerPaymentId: string | null;
  } | null,
): string {
  if (!payment) return "—";
  if (payment.provider === "RAZORPAY") return "Razorpay";
  if (payment.mockTxnId?.startsWith("WB_MOCK_"))
    return "Demo / simulated checkout";
